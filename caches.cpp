...
int Cache::saCache(int associativity){
  ...
  int rowLength = 3 * associativity;             // Rows are [VALID][TAG][DISTANCE].... 
  ...
  for (auto c = caches.begin(); c != caches.end(); c++){
    ...
    int row = rowLength / associativity;                  // Used to check for [VALID]
    ...
    if (!found){                                          // If there wasn't a match, we need to 
      ...
      for (int i = 0; i < rowLength; i += row ){          // least recently used.
	if (cacheRow[i] == 0){
	  cacheRow[i] = 1;
	  cacheRow[i + 1] = tag;
...
int Cache::saNoAlloc(int entries){
  ...
  int rowLength = 3 * entries;                   // Rows are [VALID][TAG][DISTANCE]....
  int offset = log2(cacheEntries) + log2(SIZE);  // Offset is calculated the same way.
